...
@Injectable()
export class DinhMucTienAnService {
  constructor(
    @InjectRepository(DinhMucTienAn)
    private readonly repo: Repository<DinhMucTienAn>,
  ...
  ): Promise<PaginatedResult<DinhMucTienAn>> {
  ...
  async findAll(): Promise<DinhMucTienAn[]> {
    return this.repo.find({
      where: { isActive: true, ...this.getTenantFilter() },
    });
  }

  async findOne(id: string): Promise<DinhMucTienAn> {
    const { ObjectId } = await import('mongodb');
    const item = await this.repo.findOne({
      where: { _id: new ObjectId(id) as any },
    });
    if (!item)
      throw new NotFoundException(`Không tìm thấy DinhMucTienAn với ID ${id}`);
    return item;
  }

  async findByCode(code: string): Promise<DinhMucTienAn | null> {
    return this.repo.findOne({
      where: { code, isActive: true, ...this.getTenantFilter() },
    });
  }

  async create(dto: CreateDinhMucTienAnDto): Promise<DinhMucTienAn> {
  ...
  async update(
    id: string,
    dto: UpdateDinhMucTienAnDto,
  ): Promise<DinhMucTienAn> {
  ...
  async deleteBatch(ids: string[]): Promise<SoftDeleteBatchResult> {
    return softDeleteBatch(
      this.repo as unknown as MongoRepository<DinhMucTienAn>,
      ids,
    );
  }
