...
export class CrudHandler extends CSubHanlder {
  ...
  async create(params: { data: Omit<NhomQuanLy, 'id'> }): Promise<NhomQuanLy> {
    this.setState("loading", true);
    try {
      const result = await nhomQuanLyService.create(params.data);
  ...
  async update(params: { id: string; data: Partial<NhomQuanLy> }): Promise<NhomQuanLy> {
    this.setState("loading", true);
    try {
      const result = await nhomQuanLyService.update(params.id, params.data);
      await this.executeEvent("refresh", {});
      return result;
    } finally {
      this.setState("loading", false);
  ...
  async remove(params: { id: string }): Promise<void> {
    this.setState("loading", true);
    try {
      await nhomQuanLyService.remove(params.id);
      await this.executeEvent("refresh", {});
    } finally {
      this.setState("loading", false);
    }
