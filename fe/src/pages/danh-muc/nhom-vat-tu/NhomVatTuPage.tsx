...
import { nhomVatTuImportConfig } from "@/components/import-danh-muc/configs";
import { ExportDanhMucButton, ExportDanhMucConfig } from "@/components/export-danh-muc";

const { Text } = Typography;
const { TextArea } = Input;

const nhomVatTuSchema = z.object({
  ma: z
    .string()
    .trim()
...
const NhomVatTuPage: React.FC = () => {
  const { canCreate, canEdit, canDelete, canExport } = usePagePermission("/danh-muc/nhom-vat-tu");
  const [data, setData] = useState<NhomVatTu[]>([]);
  const [loading, setLoading] = useState(false);
  const [searchText, setSearchText] = useState("");
  const [modalVisible, setModalVisible] = useState(false);
  const [editingRecord, setEditingRecord] = useState<NhomVatTu | null>(null);
  ...
  const exportConfig: ExportDanhMucConfig = useMemo(() => ({
    ...
    fetchData: async () => {
      const result = await nhomVatTuService.getPaginated({ limit: 10000 });
  ...
  const fetchData = async (
    ...
    try {
      ...
      if (result.data.length === 0 && page > 1 && result.meta.totalPages >= 1) {
        ...
        const newResult = await nhomVatTuService.getPaginated({
          page: newPage,
          limit: pageSize,
          search: search || undefined,
        });
  ...
  const handleEdit = (record: NhomVatTu) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const validated = nhomVatTuSchema.parse(values);

      const maExists = await nhomVatTuService.checkMaExists(
        validated.ma,
        editingRecord?.id
      );
      if (maExists) {
        message.error("Mã nhóm vật tư đã tồn tại");
        return;
      }

      if (editingRecord) {
        await nhomVatTuService.update(editingRecord.id, validated);
        message.success("Cập nhật nhóm vật tư thành công");
      } else {
        await nhomVatTuService.create(validated as Omit<NhomVatTu, "id">);
  ...
  const handleDelete = async (id: string) => {
    try {
      await nhomVatTuService.remove(id);
      message.success("Xóa nhóm vật tư thành công");
      fetchData(pagination.current, pagination.pageSize, searchText);
    } catch {
      message.error("Không thể xóa nhóm vật tư");
    }
  };
  ...
  const columns = [
    {
      ...
      key: "ma",
      width: 120,
      sorter: (a: NhomVatTu, b: NhomVatTu) => a.ma.localeCompare(b.ma),
      render: (text: string) => (
    ...
    },
    {
      ...
      key: "ten",
      ellipsis: true,
      sorter: (a: NhomVatTu, b: NhomVatTu) => a.ten.localeCompare(b.ten),
    },
    {
      title: "Mô tả",
      dataIndex: "moTa",
    ...
    },
    {
      ...
      render: (_: unknown, record: NhomVatTu) => (
        <Space size="small">
          {canEdit && (
            <Tooltip title="Sửa">
              <Button
  ...
  return (
    <div className="space-y-3">
      <Card>
        <FilterBar
          search={{
            ...
            placeholder: "Tìm kiếm theo mã hoặc tên nhóm vật tư...",
            width: 400,
          }}
          actions={
            <>
              <ImportDanhMucButton
                config={nhomVatTuImportConfig}
                canCreate={canCreate}
        ...
        <Table
          ...
          pagination={{
            ...
            showTotal: (total, range) =>
              `${range[0]}-${range[1]} của ${total} nhóm vật tư`,
      ...
      <Modal
        title={editingRecord ? "Sửa nhóm vật tư" : "Thêm nhóm vật tư mới"}
        open={modalVisible}
