...
const PhieuThuPage: React.FC = () => {
  const { canCreate, canEdit, canDelete, canExport } = usePagePermission("/chung-tu/phieu-thu");
  ...
  const handleSearch = async (value: string) => {
    if (value.trim()) {
      setLoading(true);
      try {
        const result = await phieuThuService.search(value);
        setData(result);
        setPagination({
          current: 1,
  ...
  const openModal = (record?: ChungTu) => {
    if (record) {
      ...
      form.setFieldsValue({
        ...
        taiKhoanNo: defaultQuyChuan?.taiKhoanNo || "111",
        taiKhoanCo: defaultQuyChuan?.taiKhoanCo || "131",
      });
    }
  ...
  const handleDoiTuongChange = (value: string) => {
    ...
    if (doiTuong) {
      ...
      form.setFieldsValue({ doiTuongTen: undefined, doiTuongSnapshot: undefined });
    }
  };

  const handleDoiTuong2Change = (value: string) => {
    const doiTuong = doiTuongList.find((d) => d.id === value);
    if (doiTuong) {
      form.setFieldsValue({
        doiTuong2Ten: doiTuong.ten,
        doiTuong2Snapshot: buildDoiTuongSnapshot(doiTuong),
      });
    } else {
      form.setFieldsValue({ doiTuong2Ten: undefined, doiTuong2Snapshot: undefined });
    }
  };
  ...
  const handleDuAnChange = (value: string) => {
    ...
    if (duAn) {
      ...
      form.setFieldsValue({ duAnTen: undefined, chuDauTuTen: undefined, duAnSnapshot: undefined });
    }
  };
  ...
  const handleNhanVienChange = (value: string) => {
    ...
    if (nhanVien) {
      ...
      form.setFieldsValue({ nhanVienTen: undefined, nhanVienSnapshot: undefined });
    }
  };
  ...
  const handleSanPhamChange = (value: string) => {
    ...
    if (sanPham) {
      ...
      form.setFieldsValue({ sanPhamTen: undefined, sanPhamSnapshot: undefined });
    }
  };
  ...
  const handleDongTienChange = (value: string) => {
    ...
    if (dongTien) {
      ...
      form.setFieldsValue({ dongTienTen: undefined, dongTienSnapshot: undefined });
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      const validation = phieuThuSchema.safeParse(submitData);
      ...
      if (editingRecord) {
        ...
        await phieuThuService.update(editingRecord.id, updateData);
        message.success("Cập nhật phiếu thu thành công");
      } else {
        await phieuThuService.create(submitData);
  ...
  const handleDelete = async (id: string) => {
    try {
      setLoading(true);
      await phieuThuService.remove(id);
  ...
  const handleSubmitForApproval = async (id: string) => {
    try {
      setLoading(true);
      await phieuThuService.submitForApproval(id);
  ...
  const columns = [
    ...
    },
    {
      ...
      ellipsis: true,
      render: (_: unknown, record: ChungTu) => {
        const ten = getDoiTuong2Ten(record);
        return ten || <Text type="secondary">-</Text>;
      },
    },
    {
    ...
    },
    {
      ...
      onFilter: (value: string | number | boolean | React.Key, record: ChungTu) => record.trangThai === value,
      ...
      >
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-2">
            <FileTextOutlined className="text-2xl animate-pulse" />
            <Title level={3} className="!text-white !mb-0">
              Phiếu Thu
            </Title>
          </div>
          <Text className="text-white/80">
            Quản lý phiếu thu tiền mặt và chuyển khoản
          </Text>
        </div>
      </div>
      ...
      <Modal
        title={
          <div className="flex items-center gap-2">
            ...
            <span>{editingRecord ? "Sửa phiếu thu" : "Tạo phiếu thu mới"}</span>
          </div>
        }
        open={modalVisible}
      ...
      >
        <Form form={form} layout="vertical" className="mt-4">
            ...
            <Col span={8}>
              ...
              >
                <InputNumber<number>
                  ...
                  parser={(value) =>
                    (value ? Number(value.replace(/\$\s?|(,*)/g, "")) : 0) as number
                  }
                  addonAfter="VNĐ"
                />
              </Form.Item>
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item name="doiTuong2Id" label="Đối tượng có">
                <Select
                  showSearch
                  allowClear
          ...
          <Form.Item name="doiTuongTen" hidden><Input /></Form.Item>
          <Form.Item name="doiTuong2Ten" hidden><Input /></Form.Item>

          <Divider>Phân bổ</Divider>
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item name="sanPhamId" label="Sản phẩm/Dịch vụ">
                <Select
                  showSearch
                  allowClear
          ...
          >
            <Input.TextArea autoSize={{ minRows: 2, maxRows: 4 }} placeholder="Nội dung phiếu thu" />
          </Form.Item>
          ...
          <Form.Item name="ghiChu" label="Ghi chú">
            <Input.TextArea autoSize={{ minRows: 2, maxRows: 4 }} placeholder="Ghi chú thêm (nếu có)" />
          </Form.Item>

          {/* Hidden snapshot fields */}
          <Form.Item name="doiTuongSnapshot" hidden><Input /></Form.Item>
          <Form.Item name="doiTuong2Snapshot" hidden><Input /></Form.Item>
          <Form.Item name="duAnSnapshot" hidden><Input /></Form.Item>
          <Form.Item name="boPhanSnapshot" hidden><Input /></Form.Item>
          <Form.Item name="doiSnapshot" hidden><Input /></Form.Item>
          <Form.Item name="nhanVienSnapshot" hidden><Input /></Form.Item>
          <Form.Item name="sanPhamSnapshot" hidden><Input /></Form.Item>
          <Form.Item name="dongTienSnapshot" hidden><Input /></Form.Item>
        </Form>
      </Modal>
