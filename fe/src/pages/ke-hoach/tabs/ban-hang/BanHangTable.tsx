...
const KHOA_RONG_COT = "kh-rong-cot-ban-hang";
...
export const BanHangTable: React.FC = () => {
  ...
  /** Gộp thêm mã nhóm có thật trên danh mục Sản phẩm — xem `nhomTuDanhMuc`. */
  const nhomOptions = useMemo(
    () => dungNhomOptions(nhomSanPhamList, sanPhamList),
    [nhomSanPhamList, sanPhamList],
  );
    ...
    ...ghimTrai<Hang>([
      {
        ...
        render: (_: unknown, row: Hang) => {
          ...
          if (!row.dong?.tam) {
            return (
              <span>{tenSanPham.get(row.dong!.val.sanPhamId)?.ma ?? "-"}</span>
            );
          }
          return (
            <Select
          ...
          return (
            <Select
              ...
              onChange={(v) =>
                handler.executeEvent("suaO", {
                  id: row.dong!.id,
                  // Tự điền nhóm theo sản phẩm vừa chọn — khỏi phải chọn hai lần.
                  patch: { sanPhamId: v, nhomMa: nhomCuaMuc(sanPhamList, v) },
                })
              }
            />
          );
